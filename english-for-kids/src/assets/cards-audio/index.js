import angry from './angry.mp3';
import argue from './argue.mp3';
import big from './big.mp3';
import bird from './bird.mp3';
import blouse from './blouse.mp3';
import boot from './boot.mp3';
import build from './build.mp3';
import carry from './carry.mp3';
import catch1 from './catch.mp3';
import cat from './cat.mp3';
import chicken from './chicken.mp3';
import chick from './chick.mp3';
import coat from './coat.mp3';
import correct from './correct.mp3';
import cry from './cry.mp3';
import dance from './dance.mp3';
import dive from './dive.mp3';
import dog from './dog.mp3';
import dolphin from './dolphin.mp3';
import draw from './draw.mp3';
import dress from './dress.mp3';
import drive from './drive.mp3';
import drop from './drop.mp3';
import error1 from './error.mp3';
import failure from './failure.mp3';
import fast from './fast.mp3';
import fish from './fish.mp3';
import fishing from './fishing.mp3';
import fly from './fly.mp3';
import friendly from './friendly.mp3';
import frog from './frog.mp3';
import giraffe from './giraffe.mp3';
import happy from './happy.mp3';
import horse from './horse.mp3';
import hug from './hug.mp3';
import jump from './jump.mp3';
import laugh from './laugh.mp3';
import lion from './lion.mp3';
import mouse from './mouse.mp3';
import old from './old.mp3';
import open1 from './open.mp3';
import pants from './pants.mp3';
import pig from './pig.mp3';
import play from './play.mp3';
import point from './point.mp3';
import pull from './pull.mp3';
import push from './push.mp3';
import rabbit from './rabbit.mp3';
import ride from './ride.mp3';
import run from './run.mp3';
import sad from './sad.mp3';
import scared from './scared.mp3';
import sheep from './sheep.mp3';
import shirt from './shirt.mp3';
import shoe from './shoe.mp3';
import sing from './sing.mp3';
import skip from './skip.mp3';
import skirt from './skirt.mp3';
import slow from './slow.mp3';
import small from './small.mp3';
import smile from './smile.mp3';
import success from './success.mp3';
import surprised from './surprised.mp3';
import swim from './swim.mp3';
import tired from './tired.mp3';
import turtle from './turtle.mp3';
import unfriendly from './unfriendly.mp3';
import young from './young.mp3';
import table from './table.mp3';
import lamp from './lamp.mp3';
import door from './door.mp3';
import cupboard from './cupboard.mp3';
import fridge from './fridge.mp3';
import window from './window.mp3';
import armchair from './armchair.mp3';
import television from './television.mp3';
import sidewalk from './sidewalk.mp3';
import road from './road.mp3';
import tree from './tree.mp3';
import grass from './grass.mp3';
import river from './river.mp3';
import bus from './bus.mp3';
import train from './train.mp3';
import sun from './sun.mp3';

export {
  angry,
  argue,
  big,
  bird,
  blouse,
  boot,
  build,
  carry,
  catch1,
  cat,
  chicken,
  chick,
  coat,
  correct,
  cry,
  dance,
  dive,
  dog,
  dolphin,
  draw,
  dress,
  drive,
  drop,
  error1,
  failure,
  fast,
  fish,
  fishing,
  fly,
  friendly,
  frog,
  giraffe,
  happy,
  horse,
  hug,
  jump,
  laugh,
  lion,
  mouse,
  old,
  open1,
  pants,
  pig,
  play,
  point,
  pull,
  push,
  rabbit,
  ride,
  run,
  sad,
  scared,
  sheep,
  shirt,
  shoe,
  sing,
  skip,
  skirt,
  slow,
  small,
  smile,
  success,
  surprised,
  swim,
  tired,
  turtle,
  unfriendly,
  young,
  table,
  lamp,
  door,
  cupboard,
  fridge,
  window,
  armchair,
  television,
  sidewalk,
  road,
  tree,
  grass,
  river,
  bus,
  train,
  sun
}
